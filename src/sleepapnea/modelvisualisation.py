...
def main():
    ...
    filter_number = 7   # change to the particular filter you want to visulaise

    x = np.linspace(0.0, N1*T, N1)
    f = filters1[:, :, filter_number]
    fig, ax = plt.subplots()
    plt.subplot(2, 1, 1)
    ...
    for i in range(40):
        f = filters2[:, filter_number, i]
        plt.subplot(10, 4, (i+1))
        plt.plot(f, color='xkcd:azure')
    plt.figure()

    for i in range(40):
        y = filters2[:, filter_number, i]
