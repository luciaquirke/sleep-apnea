...
annotationDict = defaultdict(lambda: 6, {
    'H': 0,
    'HA': 0,
    'OA': 0,
    'CA': 0,
...
for recordNumber, record in enumerate(recordList):
    ...
    # 6 shot classification target: first digit is apnea, next five are W, N1, N2, N3, and REM
    target = np.zeros((numberAnnotations, 6)).astype(int)

    for i, note in enumerate(annsamp.aux_note):
        labels = note.split(' ')
        for label in labels:
            if annotationDict[label] is not 6:
